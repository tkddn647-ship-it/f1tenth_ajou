...
class FakeSensorPublisher(Node):
    ...
    def _ray_to_walls(self, x0: float, y0: float, theta: float, max_range: float):
        c = math.cos(theta)
        s = math.sin(theta)
        eps = 1e-9
        hits = []
    ...
    def _publish_scan(self, x: float, y: float, yaw: float):
        ...
        for i in range(num_points):
            ...
            dist = self._ray_to_walls(x, y, world_theta, msg.range_max)
            ranges.append(dist)
    ...
    def _on_timer(self):
        # Smooth circular trajectory for stable fake mapping.
        radius = 2.5
        omega = 0.12
